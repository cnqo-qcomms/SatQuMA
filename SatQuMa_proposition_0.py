...
class Key_Rate_Calculator():
    ...
    def get_system_efficiency(self):
        "η_sys = η_ch*η_bob"
        system_efficiency = self.channel_efficiency*self.detector_efficiency
    ...
    def get_expecting_detection_rate_excluding_after_pulse(self, intensity):
        ...
        expecting_detection_rate_excluding_after_pulse = (1 - (1 - 2*self.dark_count_probability)*np.exp(-self.get_system_efficiency()*intensity))
    ...
    def get_bit_error_probability(self, intensity):
        ...
        bit_error_probability = (self.dark_count_probability
                                 + self.error_rate_due_to_optical_errors*(1 - np.exp(-self.get_system_efficiency()*intensity))
                                 + 0.5*self.after_pulse_probability*self.get_expecting_detection_rate_including_after_pulse(intensity))
        return bit_error_probability
    
    def get_number_of_events(self, intensity, basis):
        "n_j_k = ..."
        number_of_events = self.get_basis_selection_probability(basis)*self.get_basis_selection_probability(basis)*self.number_of_pulses*self.get_intensity_selection_probability(intensity)*self.get_expecting_detection_rate_including_after_pulse(intensity)
    ...
    def get_number_of_errors(self, intensity, basis):
        "m_j_k = ..."
        number_of_errors = self.get_bit_error_probability(intensity)*self.get_number_of_events(intensity, basis)/self.get_expecting_detection_rate_including_after_pulse(intensity)
    ...
    def get_approximate_number_of_vacuum_events(self, basis):
        "s_j_0 = ..."
        ...
        approximate_number_of_vacuum_events = max(numerator/denominator, 1e-10)
        return approximate_number_of_vacuum_events
    
    def get_approximate_number_of_single_photon_events(self, basis):
        "s_j_1 = ..."
        numerator = self.get_n_photon_state_transmission_probability(1)*self.intensity_1*(self.get_number_of_events_lowerbound(self.intensity_2, basis)
                                                                                          ...
                                                                                          - (self.intensity_2**2 - self.intensity_3**2)*(self.get_number_of_events_upperbound(self.intensity_1, basis) 
                                                                                                                                          - self.get_approximate_number_of_vacuum_events(basis)/self.get_n_photon_state_transmission_probability(0))/self.intensity_1**2)
        denominator = self.intensity_1*(self.intensity_2 - self.intensity_3) - self.intensity_2**2 + self.intensity_3**2
        approximate_number_of_single_photon_events = max(numerator/denominator, 1e-10)
        return approximate_number_of_single_photon_events
    ...
    def get_approximate_number_of_bit_error(self, basis):
        ...
        denominator = self.intensity_2 - self.intensity_3
        approximate_number_of_bit_error = min(max(numerator/denominator, 1e-10), 
                                              self.get_post_processing_block_size_2("z"))
    ...
    def get_phase_error_rate(self):
        ...
        num_min = float_info.epsilon
        ratio = min(self.get_approximate_number_of_bit_error("z")/self.get_approximate_number_of_single_photon_events("z"), 
                     num_min)
        phase_error_rate = (self.get_approximate_number_of_bit_error("z")/self.get_approximate_number_of_single_photon_events("z")
                            + gamma(self.secrecy_error,
                                   ratio,
                                    self.get_approximate_number_of_single_photon_events("z"),
                                    self.get_approximate_number_of_single_photon_events("x")))
        phase_error_rate = min(phase_error_rate, 0.5)
    ...
    def get_secret_key_length(self):
        "l = ..."
        secret_key_length = (self.get_approximate_number_of_vacuum_events("x")
                              + self.get_approximate_number_of_single_photon_events("x")
                              - self.get_approximate_number_of_single_photon_events("x")*get_binary_entropy(self.get_phase_error_rate())
                              - self.get_estimated_number_of_bits_for_error_correction("x")
                              - 6*np.log2(21/self.secrecy_error)
                              - np.log2(2/self.correctness_error))
        secret_key_length = max(secret_key_length, 0)
